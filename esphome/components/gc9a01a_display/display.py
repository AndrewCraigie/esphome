...
gc9a01a_display_ns = cg.esphome_ns.namespace("gc9a01a_display")
GC9A01ADisplay = gc9a01a_display_ns.class_(
    "GC9A01ADisplay",
    cg.PollingComponent,
    spi.SPIDevice,
    display.Display,
...
GC9A01ADisplayPreset = gc9a01a_display_ns.class_("GC9A01ADisplayPreset", GC9A01ADisplay)

PixelMode = gc9a01a_display_ns.enum("PixelMode")
PIXEL_MODES = {
    "16bit": PixelMode.PIXEL_MODE_16,
    "18bit": PixelMode.PIXEL_MODE_18,
}

GC9A01AColorMode = gc9a01a_display_ns.enum("GC9A01AColorMode")
ColorOrder = display.display_ns.enum("ColorMode")

MODELS = {
    "GC9A01A_DISPLAY": GC9A01ADisplay,
}
...
CONFIG_SCHEMA = cv.All(
    display.FULL_DISPLAY_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(GC9A01ADisplay),
...
def final_validate(config):
    ...
    return spi.final_validate_device_schema(
        "gc9a01a_display", require_miso=False, require_mosi=True
    )
...
async def to_code(config):
    ...
    if CONF_LAMBDA in config:
        ...
        cg.add_global(cg.RawStatement('ESP_LOGD("display.py", "set_writer() called and buffer should be initialized.");'))
    ...
    if config[CONF_COLOR_PALETTE] == "GRAYSCALE":
        cg.add(var.set_buffer_color_mode(GC9A01AColorMode.BITS_8_INDEXED))
        rhs = []
        for x in range(256):
        ...
        cg.add(var.set_buffer_color_mode(GC9A01AColorMode.BITS_8))
    else:
        cg.add(var.set_buffer_color_mode(GC9A01AColorMode.BITS_16))
