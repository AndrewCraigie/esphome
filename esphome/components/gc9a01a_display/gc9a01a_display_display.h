...
#include "gc9a01a_display_defines.h"
#include "gc9a01a_display_init.h"

namespace esphome {
namespace gc9a01a_display  {

static const char *const TAG = "gc9a01a_display";
const size_t GC9A01A_TRANSFER_BUFFER_SIZE = 126;  // ensure this is divisible by 6

enum GC9A01AColorMode  {
  BITS_8 = 0x08,
  BITS_8_INDEXED = 0x09,
  BITS_16 = 0x10,
};

enum GC9A01APixelMode {
...
class GC9A01ADisplay : public display::DisplayBuffer,
 ...
 public:
  GC9A01ADisplay() = default;
  GC9A01ADisplay(uint8_t const *init_sequence, int16_t width, int16_t height)
      : init_sequence_{init_sequence}, width_{width}, height_{height} {
    uint8_t cmd, num_args, bits;
    const uint8_t *addr = init_sequence;
    while ((cmd = *addr++) != 0) {
      num_args = *addr++;
      if (num_args == GC9A01A_DISPLAY_DELAY_FLAG)
        continue;
      bits = *addr;
      switch (cmd) {
        case GC9A01A_DISPLAY_MADCTL: {
  ...
  void set_buffer_color_mode(GC9A01AColorMode color_mode) { this->buffer_color_mode_ = color_mode; }
  ...
  void set_pixel_mode(GC9A01APixelMode mode) { this->pixel_mode_ = mode; }
  ...
  void setup() override;
  void on_shutdown() override { this->command(GC9A01A_DISPLAY_SLPIN); }
 ...
 protected:
  ...
  GC9A01AColorMode buffer_color_mode_{BITS_16};
...
class GC9A01ADisplayPreset : public GC9A01ADisplay {
 public:
  GC9A01ADisplayPreset() : GC9A01ADisplay(INITCMD_GC9A01A, 240, 240) {}
};
