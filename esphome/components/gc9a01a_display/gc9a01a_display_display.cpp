#include "gc9a01a_display_display.h"
...
void GC9A01ADisplay::set_madctl() {
  ...
  if (this->mirror_y_)
    mad |= MADCTL_MY;
  this->command(GC9A01A_DISPLAY_MADCTL);
  this->data(mad);
...
void GC9A01ADisplay::setup() {
  ...
  this->command(GC9A01A_DISPLAY_SLPOUT);
  delay(120);  // Matches Arduino demo implementation
  this->command(GC9A01A_DISPLAY_DISPON);
...
void GC9A01ADisplay::setup_pins_() {
...
float GC9A01ADisplay::get_setup_priority() const { return setup_priority::HARDWARE; }
...
void HOT GC9A01ADisplay::draw_absolute_pixel_internal(int x, int y, Color color) {
...
void GC9A01ADisplay::display_() {
  ...
  size_t mw_time = (w * h * 16) / mhz + w * h * 2 / GC9A01A_TRANSFER_BUFFER_SIZE * SPI_SETUP_US;
  ESP_LOGV(TAG,
  ...
  if (this->buffer_color_mode_ == BITS_16 && !this->is_18bitdisplay_ && sw_time < mw_time) {
    ...
    uint8_t transfer_buffer[GC9A01A_TRANSFER_BUFFER_SIZE];
...
void GC9A01ADisplay::draw_pixels_at(int x_start, int y_start, int w, int h, const uint8_t *ptr,
  ...
  if (!this->is_18bitdisplay_) {
    ...
    // 18 bit mode
    uint8_t transfer_buffer[GC9A01A_TRANSFER_BUFFER_SIZE * 4];
    ESP_LOGV(TAG, "Doing multiple write");
    size_t rem = h * w;  // remaining number of pixels to write
    size_t idx = 0;      // index into transfer_buffer
    size_t pixel = 0;    // pixel number offset
...
uint32_t GC9A01ADisplay::get_buffer_length_() { return this->get_width_internal() * this->get_height_internal(); }

void GC9A01ADisplay::command(uint8_t value) {
  this->start_command_();
  this->write_byte(value);
  this->end_command_();
}

void GC9A01ADisplay::data(uint8_t value) {
  this->start_data_();
  this->write_byte(value);
  this->end_data_();
}

void GC9A01ADisplay::send_command(uint8_t command_byte, const uint8_t *data_bytes, uint8_t num_data_bytes) {
...
void GC9A01ADisplay::start_command_() {
  this->dc_pin_->digital_write(false);
  this->enable();
}
void GC9A01ADisplay::start_data_() {
  this->dc_pin_->digital_write(true);
  this->enable();
}

void GC9A01ADisplay::end_command_() { this->disable(); }
void GC9A01ADisplay::end_data_() { this->disable(); }

void GC9A01ADisplay::reset_() {
  if (this->reset_pin_ != nullptr) {
...
void GC9A01ADisplay::init_lcd_(const uint8_t *addr) {
  if (addr == nullptr)
    return;
  uint8_t cmd, x, num_args;
  while ((cmd = *addr++) != 0) {
    x = *addr++;
    if (x == GC9A01A_DISPLAY_DELAY_FLAG) {
      cmd &= 0x7F;
...
void GC9A01ADisplay::set_addr_window_(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
  ...
  y1 += this->offset_y_;
  y2 += this->offset_y_;
  this->command(GC9A01A_DISPLAY_CASET);
  this->data(x1 >> 8);
  this->data(x1 & 0xFF);
  this->data(x2 >> 8);
  this->data(x2 & 0xFF);
  this->command(GC9A01A_DISPLAY_PASET);  // Page address set
  this->data(y1 >> 8);
  this->data(y1 & 0xFF);
  this->data(y2 >> 8);
  this->data(y2 & 0xFF);
  this->command(GC9A01A_DISPLAY_RAMWR);  // Write to RAM
  this->start_data_();
}

void GC9A01ADisplay::invert_colors(bool invert) {
  this->pre_invertcolors_ = invert;
  if (is_ready()) {
    this->command(invert ? GC9A01A_DISPLAY_INVON : GC9A01A_DISPLAY_INVOFF);
  }
}

int GC9A01ADisplay::get_width_internal() { return this->width_; }
int GC9A01ADisplay::get_height_internal() { return this->height_; }
