...
gc9a01acustom_ns = cg.esphome_ns.namespace("gc9a01acustom")
GC9A01ACUSTOMDisplay = gc9a01acustom_ns.class_(
    "GC9A01ACUSTOMDisplay",
    cg.PollingComponent,
    spi.SPIDevice,
    display.Display,
...
PixelMode = gc9a01acustom_ns.enum("PixelMode")
PIXEL_MODES = {
...
CONFIG_SCHEMA = cv.All(
    display.FULL_DISPLAY_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(GC9A01ACUSTOMDisplay),
...
def final_validate(config):
    ...
    return spi.final_validate_device_schema(
        "gc9a01acustom", require_miso=False, require_mosi=True
    )
...
async def to_code(config):
    ...
    if config[CONF_COLOR_PALETTE] == "GRAYSCALE":
        cg.add(var.set_buffer_color_mode(GC9A01ACUSTOMColorMode.BITS_8_INDEXED))
        rhs = []
        for x in range(256):
        ...
        cg.add(var.set_buffer_color_mode(GC9A01ACUSTOMColorMode.BITS_8))
    else:
        cg.add(var.set_buffer_color_mode(GC9A01ACUSTOMColorMode.BITS_16))
