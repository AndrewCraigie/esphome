...
#include "gc9a01acustom_defines.h"
#include "gc9a01acustom_init.h"

namespace esphome {
namespace gc9a01acustom {

static const char *const TAG = "gc9a01acustom";
const size_t GC9A01ACUSTOM_TRANSFER_BUFFER_SIZE = 126;  // ensure this is divisible by 6
...
class GC9A01ACUSTOMDisplay : public display::DisplayBuffer,
 ...
 public:
  GC9A01ACUSTOMDisplay() = default;
  GC9A01ACUSTOMDisplay(uint8_t const *init_sequence, int16_t width, int16_t height)
      : init_sequence_{init_sequence}, width_{width}, height_{height} {
    uint8_t cmd, num_args, bits;
    const uint8_t *addr = init_sequence;
    while ((cmd = *addr++) != 0) {
      num_args = *addr++;
      if (num_args == GC9A01ACUSTOM_DELAY_FLAG)
        continue;
      bits = *addr;
      switch (cmd) {
        case GC9A01ACUSTOM_MADCTL: {
...
//-----------   GC9A01ACUSTOM_240_TFT display --------------
class GC9A01ACustom240TFT : public GC9A01ACUSTOMDisplay {
 public:
  GC9A01ACustom240TFT() : GC9A01ACUSTOMDisplay(INITCMD_GC9A01A, 240, 240) {}
};
