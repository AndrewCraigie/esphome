...
void GC9A01ACUSTOMDisplay::set_madctl() {
  ...
  if (this->mirror_y_)
    mad |= MADCTL_MY;
  this->command(GC9A01ACUSTOM_MADCTL);
  this->data(mad);
  ...
  switch (this->pixel_mode_) {
    case PIXEL_MODE_16:
      if (this->is_18bitdisplay_) {
        this->command(GC9A01ACUSTOM_PIXFMT);
        this->data(0x55);
...
void GC9A01ACUSTOMDisplay::dump_config() {
  LOG_DISPLAY("", "gc9a01acustom", this);
...
void HOT GC9A01ACUSTOMDisplay::draw_absolute_pixel_internal(int x, int y, Color color) {
...
void GC9A01ACUSTOMDisplay::display_() {
  ...
  size_t mw_time = (w * h * 16) / mhz + w * h * 2 / GC9A01ACUSTOM_TRANSFER_BUFFER_SIZE * SPI_SETUP_US;
  ESP_LOGV(TAG,
  ...
  if (this->buffer_color_mode_ == BITS_16 && !this->is_18bitdisplay_ && sw_time < mw_time) {
    ...
    uint8_t transfer_buffer[GC9A01ACUSTOM_TRANSFER_BUFFER_SIZE];
...
void GC9A01ACUSTOMDisplay::draw_pixels_at(int x_start, int y_start, int w, int h, const uint8_t *ptr,
  ...
  if (!this->is_18bitdisplay_) {
    ...
    // 18 bit mode
    uint8_t transfer_buffer[GC9A01ACUSTOM_TRANSFER_BUFFER_SIZE * 4];
    ESP_LOGV(TAG, "Doing multiple write");
    size_t rem = h * w;  // remaining number of pixels to write
    size_t idx = 0;      // index into transfer_buffer
    size_t pixel = 0;    // pixel number offset
...
uint32_t GC9A01ACUSTOMDisplay::get_buffer_length_() { return this->get_width_internal() * this->get_height_internal(); }
...
void GC9A01ACUSTOMDisplay::start_command_() {
  this->dc_pin_->digital_write(false);
  this->enable();
}
void GC9A01ACUSTOMDisplay::start_data_() {
  this->dc_pin_->digital_write(true);
  this->enable();
}

void GC9A01ACUSTOMDisplay::end_command_() { this->disable(); }
void GC9A01ACUSTOMDisplay::end_data_() { this->disable(); }

void GC9A01ACUSTOMDisplay::reset_() {
  if (this->reset_pin_ != nullptr) {
...
void GC9A01ACUSTOMDisplay::dump_debug_info() {
  this->dump_config();
  ESP_LOGI(TAG, "=== GC9A01ACUSTOM Display Debug Info V4 ===");
...
void GC9A01ACUSTOMDisplay::set_addr_window_(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2) {
  ...
  this->data(x2 >> 8);
  this->data(x2 & 0xFF);
  this->command(GC9A01ACUSTOM_PASET);  // Page address set
  this->data(y1 >> 8);
  this->data(y1 & 0xFF);
  this->data(y2 >> 8);
  this->data(y2 & 0xFF);
  this->command(GC9A01ACUSTOM_RAMWR);  // Write to RAM
  this->start_data_();
}

void GC9A01ACUSTOMDisplay::invert_colors(bool invert) {
  this->pre_invertcolors_ = invert;
  if (is_ready()) {
    this->command(invert ? GC9A01ACUSTOM_INVON : GC9A01ACUSTOM_INVOFF);
  }
}

int GC9A01ACUSTOMDisplay::get_width_internal() { return this->width_; }
int GC9A01ACUSTOMDisplay::get_height_internal() { return this->height_; }
